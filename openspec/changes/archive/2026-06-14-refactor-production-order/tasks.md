...
- [x] 运营创建草稿 → 发布（库存充足/不足两种场景）→ 工人开工 → 工人报工 → 验证成品入库
- [x] 运营创建草稿 → 发布 → 工人开工 → 运营尝试取消（应被拒绝）
- [x] 运营创建草稿 → 发布 → 取消 → 验证物料库存退回
- [x] 工人尝试创建订单（应被拒绝）
- [x] 工人尝试发布订单（应被拒绝）
- [x] 用户有 operator + worker 角色时验证所有操作正常
- [x] 订单号格式验证（新格式 P-YYMMDD-XXXX）
